#!/usr/bin/env python3
r"""
make_demo_drone.py — generate a generic placeholder quadrotor USD (no CAD).

100% original primitive geometry, but follows the same conventions the Starling
toolchain produces, so the whole pipeline runs out-of-the-box:
  - stage: meters, Z-up, defaultPrim /World
  - /World/Drone : single rigid body (0.5 kg) + box collider + primitive visuals
  - rotor_0..3   : thrust frames with drone:spinDir (+-1, X-quad diagonals match)
  - sensor frames (tof_link / cam_*_front / cam_*_down) with drone:sensorType +
    drone:viewDir; FORWARD = -Y, DOWN = -Z, matching the real layout
...
ap.add_argument("--out", default="demo_drone.usd")
