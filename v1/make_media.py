#!/usr/bin/env python3
r"""
make_media.py — render README media for a drone USD.

  1) 6-view contact sheet (+X/-X/-Y/+Y/top/bottom) -> docs/contact_sheet.png
  2) short flight GIF (takeoff + circle, geometric controller) -> docs/flight.gif

Defaults to demo_drone.usd (original placeholder). Point --usd anywhere, but only
publish media of geometry you have the rights to.
...
ap.add_argument("--usd", default="demo_drone.usd")
...
def main():
    ...
    try:
        import imageio
        imageio.mimsave(gif, frames, duration=0.05, loop=0)   # loop=0 -> infinite
