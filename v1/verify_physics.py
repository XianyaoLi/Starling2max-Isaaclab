#!/usr/bin/env python3
r"""
verify_physics.py — drop-test drone_physics.usd on a ground plane.

Uses the asset's OWN authored physics (does not re-apply it — re-applying was
what made the drone fall through the floor). PASS = printed z settles to a small
constant (~half the box height) instead of falling without bound.

    python verify_physics.py --usd drone_physics.usd [--headless] [--spawn-z 0.5]
"""
...
ap.add_argument("--usd", default="drone_physics.usd")
