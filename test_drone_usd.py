...
Sanity-test the STEP->OBJ->USD converted drone asset (drone.usd) inside
Isaac Sim, BEFORE trying to use it in Isaac Lab.

What it checks:
  1. The USD opens and references cleanly.
  2. Prim hierarchy + how many Mesh prims exist.
  3. World-space bounding box of the whole asset, and per-top-level-part
     extents/centers  -> lets you confirm the physical SIZE and LAYOUT.
  4. Scale sanity check against the real Starling 2 Max airframe diagonal
     (0.322 m). The offline OBJ scan of this asset measured a ~3.2 m
     diagonal (~10x too big / propeller pods misplaced), so this check
     will very likely FAIL until the asset is fixed -- that's the point.
  5. Whether any UsdPhysics (rigid body / collision / mass) is authored
     (a pure geometry conversion has none).

Optional (--physics): wrap the asset in a single rigid body with convex
collision, drop it onto a ground plane and step the sim, printing the
resulting height so you can see it behaves as one body.

Run on Windows (matching your run_drone_nav.py setup):
    cd C:\isaacsim
    python.bat <path>\test_drone_usd.py --usd <path>\drone.usd
    python.bat <path>\test_drone_usd.py --usd <path>\drone.usd --physics
Linux:
    ./python.sh test_drone_usd.py --usd drone.usd

Add --headless to skip the viewport (inspection still prints).
"""
