#!/usr/bin/env python3
r"""
add_drone_frames.py  (mesh-name-driven)

Add control/sensor FRAMES to drone_physics.usd and write starling2max.usd
(ModalAI Starling 2 Max, C29).

Positions are read DIRECTLY from named geometry meshes via their world bounding
box (ComputeWorldBound) -- this is robust to the fact that every mesh's transform
pivot sits at the origin (the STEP->OBJ->USD conversion baked positions into the
vertices). Do NOT trust a mesh's Translate field; use its world bbox instead.

Discovered layout for this asset (verified in viewport):
  Front sensor module @ -Y (so FORWARD = -Y, not +X):
      Body1_75, Body1_77  -> tracking + hires (assigned by size: bigger=hires)
      ToF has no mesh; placed offset from Body1_77 (--tof-ref / --tof-offset)
  Down sensor module @ +Y, facing down (-Z):
      Body1_95, Body1_93  -> tracking + hires (bigger=hires)
  Motors: Body1_4, Body1_2, Body1_6, Body1_8 -> rotor thrust points

Frame orientation: local +X is aimed along the view direction.
  forward cams -> RotateZ so +X points along the front-module heading (-Y here)
  down cams    -> RotateY=90 so +X points -Z (down)
Each carries drone:sensorType + drone:viewDir; rotors carry drone:spinDir (+-1).

Run:
    python add_drone_frames.py --in drone_physics.usd --out starling2max.usd
    # override mesh names if needed:
    python add_drone_frames.py --front-meshes Body1_75 Body1_77 \
        --down-meshes Body1_95 Body1_93 \
        --rotor-meshes Body1_4 Body1_2 Body1_6 Body1_8 \
        --tof-ref Body1_77 --tof-offset 0.018 0 0

Verify physics unaffected:
    python verify_physics.py --usd starling2max.usd --headless
"""
...
ap.add_argument("--in", dest="src", default=os.path.join(here, "drone_physics.usd"))
ap.add_argument("--out", dest="dst", default=os.path.join(here, "starling2max.usd"))
