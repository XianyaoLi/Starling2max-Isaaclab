...
Functional test of starling2max.usd in Isaac Sim / Isaac Lab.

  TEST 1  Cameras: a camera at each of the 5 sensor frames captures a frame.
          RGB for the 4 cameras, DEPTH for the ToF (via a Replicator
          distance_to_image_plane annotator, which is the reliable path). An
          obstacle is placed a known distance in front (-Y) so the ToF depth
          can be checked. Images + depth saved to --out.

  TEST 2  Rotors: a visible 2-blade marker spins at each of the 4 rotor frames
          (direction from drone:spinDir). A closed-loop altitude PD controller
          applies thrust at the rotor points so the drone GENTLY RISES to
          --hover-z and HOLDS -- then keeps hovering while you inspect the
          viewport (Ctrl-C to quit). No more open-loop fly-away.

Orientation from the asset: FORWARD = -Y (the 3-sensor front module), DOWN = -Z.

Run:
    python test_starling_functions.py --usd starling2max.usd --out cam_test_out
    python test_starling_functions.py --test cameras
...
ap.add_argument("--out", default=os.path.join(here, "cam_test_out"))
