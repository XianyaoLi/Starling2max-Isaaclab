#!/usr/bin/env python3
r"""
add_drone_physics.py

Turn the geometry-only drone_fixed.usd into a physics-enabled SINGLE rigid
body usable in Isaac Lab, and write drone_physics.usd.

Adds:
  - /World/Drone           Xform + RigidBodyAPI + MassAPI (mass = --mass, 0.5 kg)
  - /World/Drone/visual    reference to drone_fixed.usd (the detailed meshes,
                           VISUAL ONLY -- no per-mesh collision)
  - /World/Drone/collision collider approximating the airframe, chosen by
                           --collision {box|hull|decomp}:
        box    (default) one Cube collider sized to the asset bbox. Fast/robust.
        hull   convexHull on every visual mesh. Tighter, heavy to cook (1877 meshes).
        decomp convexDecomposition on every mesh. Most accurate, slowest.

Notes:
  - This is stage A (single body). For true quadrotor RL you later split into
    base_link + 4 rotor bodies (see --list-corners to find the 4 prop clusters).
  - Inertia: with MassAPI mass set and no explicit inertia, PhysX computes the
    inertia tensor from the collision shape. For a box collider that is a decent
    first approximation; refine later from CAD if needed.

Run (full Isaac Lab):
    python add_drone_physics.py --in drone_fixed.usd --out drone_physics.usd
    python add_drone_physics.py --collision box --mass 0.5
    python add_drone_physics.py --list-corners        # just report prop clusters, no write

Verify (drops it onto a ground plane):
    python test_drone_usd.py --usd drone_physics.usd --physics
"""
...
ap.add_argument("--in", dest="src", default=os.path.join(here, "drone_fixed.usd"))
ap.add_argument("--out", dest="dst", default=os.path.join(here, "drone_physics.usd"))
